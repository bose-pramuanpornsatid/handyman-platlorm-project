...
const Leaderboard: React.FC<LeaderboardProps> = memo(({ leaderboard }) => {
  return (
    <div className="flex flex-col items-start">
      ...
      {/* Description Section */}
      <h2 className="text-sm text-gray-900 mb-4">Top 5 Schools by Total Score</h2>
      ...
      <ol className="w-full">
        {leaderboard.map((entry, index) => (
          <li
            key={entry.school_id}
            className="flex items-center py-3 px-4 m-2 space-x-4 border-2 cursor-pointer rounded-lg transition-all hover:bg-gray-100"
          >
            {/* Ranking Circle */}
            ...
            <div className="flex-1">
              ...
              <p className="text-sm text-gray-700 mt-1">Top Student: {entry.student_names}</p>
            </div>
          </li>
        ))}
      </ol>
